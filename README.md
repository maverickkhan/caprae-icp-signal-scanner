...
| The site is JavaScript-rendered or otherwise yields almost no text | **Thin-content guard:** if fewer than **600 characters** of text were extracted across all fetched pages (live or Wayback), nothing is extracted or judged — the scan is stored as **No evidence** (`no_evidence_reason = content_too_thin`, score empty, coverage 0) and the drawer says so; e.g. mrrooter.com yields ~560 characters over 4 pages. Zero readable pages is stored as `site_unreadable`. There is no headless browser by design |
...
47 scans are pre-warmed (25 on the buy-box preset, 22 on the B2B sales ICP) so the ranked table loads instantly on first visit — you don't have to wait on a cold scan to see the feature work.
...
A small deterministic backstop (`services/signals.py`) also scans every page for explicit ownership statements — "is a brand of *Org*", "subsidiary/division of *Org*", "owned/acquired by *Org LLC/Inc/…*", "company-owned locations across …", franchise wording, "family-owned", "since *YYYY*" — and emits them as exact-grounded facts with the verbatim sentence, so a corporate-affiliation red flag never depends on what the sampled LLM extraction happened to include. Generic phrases ("part of our family", "backed by our guarantee", "member of") and testimonial text never match (unit-tested in `api/tests/test_signals.py`). When any avoid criterion is present, no outreach note is written at all.
...
**Eval results** (`evals/results.md`, 7 golden domains incl. two false-positive guards, re-run 2026-09-17 after the retest fixes):

| domain | icp | score | coverage | pages | facts | exact/fuzzy/record/none | expected facts | agreement | latency |
|---|---|---|---|---|---|---|---|---|---|
| frostac.com | buybox | 100.0 | 40.0% | 6 | 26 | 24/0/1/1 | 2/2 | in_business_20_plus_years: met→met, local_or_regional_focus: met→met, pe_backed_or_group_owned: not_met\|unknown→unknown | 2.2s (cache) |
| expertairco.com | buybox | 80.0 | 60.0% | 4 | 17 | 16/0/1/0 | 2/2 | in_business_20_plus_years: met→met, founder_or_owner_operated: met→unknown ✗, dated_digital_presence: not_met→not_met | 1.5s (cache) |
| expresshvacservice.com | buybox | 100.0 | 20.0% | 6 | 16 | 12/1/1/2 | 1/1 | local_or_regional_focus: met→met, in_business_20_plus_years: unknown→unknown | 1.5s (cache) |
| shellabyair.com | sales | 50.0 | 40.0% | 3 | 9 | 7/0/1/1 | 2/2 | recent_growth_or_accolades: met→met, multiple_business_locations: not_met→not_met | 1.5s (cache) |
| justiceac.com | sales | None | 0.0% | 2 | 5 | 4/0/1/0 | 1/1 | identifiable_decision_maker: unknown→unknown | 1.5s (cache) |
| ars.com | buybox | 9.4 | 80.0% | 6 | 21 | 20/0/1/0 | 2/2 | pe_backed_or_group_owned: met→met | 1.6s (cache) |
| yell4george.com | buybox | 89.5 | 100.0% | 6 | 41 | 35/1/2/3 | 0/0 | pe_backed_or_group_owned: not_met\|unknown→not_met, is_franchise: not_met\|unknown→not_met | 1.5s (cache) |

**Summary:** 135 facts extracted, 128 grounded (95% — 118 exact, 2 fuzzy, 8 record, 7 none) · expected-fact recall 10/10 · criterion agreement vs. hand-checked expectations 13/14 (expertairco.com's "family owned business" was judged unknown for owner-operated in this run) · every `met`/`not_met` verdict carries a verified quote · the ars.com corporate-subsidiary red flag is caught (score 9.4, no outreach note) while the two false-positive guards (frostac.com, yell4george.com — independent businesses) stay unflagged · latency: 24.5 s average over 43 forced rescans on the live Vercel function (2 in parallel), ~2 s when cached.
...
The buy-box's service-area criterion can read a national locations directory as a named service area; the coverage multiplier and the franchise / PE red flags keep those companies out of the top of the ranking.
...
| Label rename | 01:30 (Sep 18) | 01:40 | Opus 5, on request: renamed the buy-box criterion label `local_or_regional_focus` from "Local or Regional Service Area" to "Named Service Area" directly in Neon (`jsonb_set` with `WITH ORDINALITY`), because the preset lock (400 on PUT/compile) is intentional and `criteria_hash` excludes `label`, so all 47 pre-warmed scans stayed valid (hash verified identical before/after; 124 rows / 25 done on the buy-box unchanged). README: pre-warm count 47 (25/22), one roadmap sentence on the service-area criterion. |
| Thin-content guard | 01:45 (Sep 18) | 02:30 | Opus 5. mrrooter.com scored 100 at 40% coverage off ~560 chars of JS-rendered text. Added THIN_CONTENT_CHARS=600: total extracted text across all usable pages is tracked per scan (`scans.content_chars`); under the threshold (or zero pages) the scan is stored with `no_evidence_reason` = content_too_thin / site_unreadable, extraction + judging are skipped, all criteria Unknown, score null, coverage 0, no note; table shows No evidence with a reason tooltip, drawer shows a banner. Threshold check against the 47 latest scans: under 600 = couvillionplumbingllc.com (0 chars), dandsplumbingtexas.com (0), valuableacheating.com (243), mdnenterprises.com (376, Wayback meta description only), mrrooter.com (559); good leads far above (expertairco 5.2k, acandplumbing 17k, vanderfordair 20k, ars 20k, yell4george 22k). Affected pairs force-rescanned on prod. |
